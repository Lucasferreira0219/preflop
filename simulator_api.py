...
class SimulatorApi:
    ...
    def submit_answer(self, user_action: str, user_id: int = 1):
        global _current_question
        ...
        stats.record(_current_question, result, user_id=user_id)

        result['buckets']        = _current_question['buckets']
        result['stats']          = _basic_stats(stats.summary(user_id=user_id))
    ...
    def get_stats(self, user_id: int = 1):
        return _basic_stats(stats.summary(user_id=user_id))

    def reset_stats(self, user_id: int = 1):
        stats.reset(user_id=user_id)
        return _basic_stats(stats.summary(user_id=user_id))

    def get_analytics(self, from_ts=None, to_ts=None, user_id: int = 1):
    ...
    def get_improvement(self, window_days=7, user_id: int = 1):
        return stats.improvement(int(window_days), user_id=user_id)
